...
class DATA_LOADER(object):
    def __init__(self, opt):
        if opt.dataset in ['CUB', 'AWA2', 'APY', 'FLO', 'SUN']:
    ...
    def read_fine_tune(self,opt):
        ...
        print('==========================================================')
        print('seenclasses:',self.seenclasses.shape, self.seenclasses)
        self.unseenclasses = torch.from_numpy(np.unique(self.test_unseen_label.numpy()))
        print('unseenclasses:',self.unseenclasses.shape, self.unseenclasses)
        print('all attribute:',self.attribute.shape,self.attribute)
        print('seen attribute:',self.train_att.shape,self.train_att)
        print('unseen attribute:',self.test_att.shape, self.test_att)
    ...
    def read_matdataset(self, opt):
        ...
        matcontent = sio.loadmat(opt.dataroot + "/" + 'ImageNet' + "/" + '100k_feature' + ".mat")
        outlier_feature = matcontent['feature']
        _outlier_feature = scaler.transform(outlier_feature)
        self.outlier_feature = torch.from_numpy(_outlier_feature).float()
        self.outlier_feature.mul_(1 / mx)
        pass
