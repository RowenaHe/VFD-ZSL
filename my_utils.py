...
class EMA:
    ...
    def reset_model(self, ema_model, model):
        ema_model.load_state_dict(model.state_dict())

    def reset_ema_average(self, ema_model, model):
        for current_param, prior_param in zip(model.parameters(), ema_model.parameters()):
            if prior_param.data is None:
                prior_param.data = current_param.data
            else:
                prior_param.data = self.beta * prior_param.data + (1 - self.beta) * current_param.data

    def step_ema(self, ema_model, model, opt=None):
        if opt!=None:
            if self.step < opt.vae_start+5000:
                self.reset_model(ema_model, model)
                self.step += 1
            else:
                self.reset_ema_average(ema_model, model)
                self.step += 1
        else:
            if self.step < 500:
                self.reset_model(ema_model, model)
...
def synthesize_feature_test(netG, dataset, opt, gen_size = None):
    if gen_size is not None:
        gen_feat = torch.FloatTensor(dataset.ntest_class * opt.nSample, gen_size)
    elif opt.gen_size is not None:
        gen_feat = torch.FloatTensor(dataset.ntest_class * opt.nSample, opt.gen_size)
    else:
    ...
    with torch.no_grad():
        for i in range(dataset.ntest_class):
            ...
            text_feat = torch.from_numpy(text_feat).to(opt.gpu)
            z = torch.randn(opt.nSample, opt.Z_dim).to(opt.gpu)
            G_sample = netG.decode(z, text_feat)#ae.encoder()[:,:opt.S_dim]
            gen_feat[i*opt.nSample:(i+1)*opt.nSample] = G_sample
            gen_label = np.hstack((gen_label, np.ones([opt.nSample])*i))
    return gen_feat, torch.from_numpy(gen_label.astype(int))
